...
class AutoNumberMixin(models.Model):
    ...
    def save_with_autonumber(self, prefix=None, *args, **kwargs):
        ...
        with transaction.atomic():
            ...
            for auto_type in self.auto_number_attrs:
                if not getattr(self, auto_type.attr_name):
                    auto_type.set_column(self, prefix=prefix)
            new_kwargs = kwargs
...
class AutoColumnType:
    ...
    def set_column(self, record, prefix=None):
...
class AutoNumberColumn(AutoColumnType):
    ...
    def set_column(self, record, prefix=None):
        ...
        if prefix is not None:
            setattr(record, self.attr_name, prefix + str(int(an_row.last_nbr) + 1))
        else:
